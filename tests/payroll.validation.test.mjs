/**
 * Payroll core calibration tests.
 *
...
const PAYSLIPS = [
  {
    ...
    cumErBase: 190 - 190, // far below the cap in July; exact value is irrelevant
  ...
  for (const ps of PAYSLIPS) {
    describe(ps.name, () => {
      const r = P.month({
        ...
        nonCash: NON_CASH,
        kup: ps.kup,
        cumErBase: ps.cumErBase,
        cumTaxBase: ps.cumTaxBase,
        cap: CAP_2026,
        ppkPct: 0,
        deductions: 0,
        netAddon: NET_ADDON
...
describe('non-cash taxable benefits', () => {
  it('raise the ZUS and PIT base but are not paid out', () => {
    ...
    const withBenefit = P.month({ ...common, gross: GROSS, nonCash: NON_CASH });
    const paidOut = P.month({ ...common, gross: GROSS, nonCash: 0 });
